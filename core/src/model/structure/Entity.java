...
public class Entity extends MappedSuperclass implements EntityWithLifeCycle, ElementInRelationship {
    ...
    @XmlTransient
    private List<Entity> childrenEntity = new ArrayList<>();
    @XmlTransient
    private List<Operation> entityOperations = new ArrayList<>();
	...
	public void setParentEntity(Entity parentEntity) {
		...
		if (parentEntity != null) {
			parentEntity.getChildrenEntity().add(this);
		}
	}

	public List<String> getIDNames() {
		List<String> result = new ArrayList<>();
		for (Id id : getAttributes().getId()) {
			if (id.getColumn() != null && id.getColumn().getName() != null && !id.getColumn().getName().isEmpty()) {
				result.add(id.getColumn().getName());
			} else {
				result.add(id.getName());
			}
		}
		if (parentEntity != null) {
			result.addAll(parentEntity.getIDNames());
		}
		return result;
	}
	...
	public Id getIdByName(String name) {
		...
		if (parentEntity != null) {
			return parentEntity.getIdByName(name);
		}
		return null;
	}
	
	public List<Id> getIDsWithParent() {
		List<Id> result = new ArrayList<>();
		result.addAll(getAttributes().getId());
		if (parentEntity != null) {
			result.addAll(parentEntity.getIDsWithParent());
		}
		return result;
	}
	
	public List<Entity> getChildrenEntity() {
	...
	public ManyToOne getCorrespondingManyToOne(String targetEntity, String mappedBy) {
		for (ManyToOne m2o : getAttributes().getManyToOne()) {
			if (m2o.getTargetEntity().equals(targetEntity) && m2o.getName().equals(mappedBy)) {
				return m2o;
			}
		}
		return null;
	...
	public List<Entity> getReferencedEntityInRelationship() {
		List<Entity> entities = new ArrayList<>();
		if (this.getParentEntity() != null && this.getParentEntity().getInheritance() != null && 
				(this.getParentEntity().getInheritance().getStrategy() == null || this.getParentEntity().getInheritance().getStrategy() == InheritanceType.SINGLE_TABLE)) {
			entities.addAll(this.getParentEntity().getReferencedEntityInRelationship());
		} else if (inheritance != null && inheritance.getStrategy() == InheritanceType.TABLE_PER_CLASS) {
			for (Entity entity : getChildrenEntity()) {
				entities.addAll(entity.getReferencedEntityInRelationship());
			}
		} else {
			entities.add(this);
		}
	...
	public List<Operation> getOperations() {
		return Collections.unmodifiableList(this.entityOperations);
	}
	
	public void addOperation(Operation operation) {
		if (operation == null) {
			throw new IllegalArgumentException("Cannot add a null operation");
		}
		this.entityOperations.add(operation);
	}

	@Override
	public String toString() {
		return "Entity [description=" + description + ", table=" + table + ", secondaryTable=" + secondaryTable
				+ ", primaryKeyJoinColumn=" + primaryKeyJoinColumn + ", primaryKeyForeignKey=" + primaryKeyForeignKey
				+ ", idClass=" + idClass + ", inheritance=" + inheritance + ", discriminatorValue=" + discriminatorValue
				+ ", discriminatorColumn=" + discriminatorColumn + ", sequenceGenerator=" + sequenceGenerator
				+ ", tableGenerator=" + tableGenerator + ", namedQuery=" + namedQuery + ", namedNativeQuery="
				+ namedNativeQuery + ", namedStoredProcedureQuery=" + namedStoredProcedureQuery
				+ ", sqlResultSetMapping=" + sqlResultSetMapping + ", excludeDefaultListeners="
				+ excludeDefaultListeners + ", excludeSuperclassListeners=" + excludeSuperclassListeners
				+ ", entityListeners=" + entityListeners + ", prePersist=" + prePersist + ", postPersist=" + postPersist
				+ ", preRemove=" + preRemove + ", postRemove=" + postRemove + ", preUpdate=" + preUpdate
				+ ", postUpdate=" + postUpdate + ", postLoad=" + postLoad + ", attributeOverride=" + attributeOverride
				+ ", associationOverride=" + associationOverride + ", convert=" + convert + ", namedEntityGraph="
				+ namedEntityGraph + ", attributes=" + attributes + ", name=" + name + ", clazz=" + clazz + ", access="
				+ access + ", cacheable=" + cacheable + ", metadataComplete=" + metadataComplete + ", isAbstract=" + isAbstract + ", parentClassName=" + parentClassName
				+ "]\n";
	}
