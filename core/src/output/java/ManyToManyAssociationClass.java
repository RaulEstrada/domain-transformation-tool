...
public class ManyToManyAssociationClass {
	private JDefinedClass associationsClass;
	private Creator creator;
	private Map<String, JDefinedClass> classesMap;
	private Entity entity;
	private ManyToMany association;
	
	/**
	 * Initializes the many to many association class.
	 ...
	 */
	public ManyToManyAssociationClass(JDefinedClass associationsClass, 
			Creator creator, Map<String, JDefinedClass> classesMap, 
			Entity entity, ManyToMany association){
	...
	private void createLink(JDefinedClass class_, JDefinedClass thisClass, 
		...
		String setNameThis = CaseFormat.LOWER_UNDERSCORE.to(
				CaseFormat.LOWER_CAMEL, "get_" + association.getName());
		setNameThis = "_" + setNameThis;
		JInvocation assign1 = arg0.invoke(setNameThis).invoke("add").arg(arg1);
		...
		if (mappedBy != null && !mappedBy.isEmpty()) {
			...
			setNameOther = "_get" + setNameOther;
			JInvocation assign2 = arg1.invoke(setNameOther).invoke("add").arg(arg0);
	...
	private void createUnlink(JDefinedClass class_, JDefinedClass thisClass, 
		...
		if (mappedBy != null && !mappedBy.isEmpty()) {
			String setNameOther = mappedBy.substring(0, 1).toUpperCase() + 
					mappedBy.substring(1);
			setNameOther = "_get" + setNameOther;
			JInvocation assign1Null = arg1.invoke(setNameOther).invoke("remove")
					.arg(arg0);
			bodyUnlink.add(assign1Null);
		}
		String setNameThis = CaseFormat.LOWER_UNDERSCORE
				.to(CaseFormat.LOWER_CAMEL, "get_" + association.getName());
		setNameThis = "_" + setNameThis;
		JInvocation assign2Null = arg0.invoke(setNameThis).invoke("remove")
				.arg(arg1);
		bodyUnlink.add(assign2Null);
	}
}
