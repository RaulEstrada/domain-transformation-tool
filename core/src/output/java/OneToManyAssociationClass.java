...
public class OneToManyAssociationClass {
	...
	/**
	 * Initializes the one to many association class.
	 ...
	 */
	public OneToManyAssociationClass(JDefinedClass associationsClass, 
			Creator creator, Map<String, JDefinedClass> classesMap, 
			Entity entity, OneToMany association){
	...
	private void link(JDefinedClass class_, JDefinedClass thisClass, 
		...
		ManyToOne manyToOne = association.getEntity().getCorrespondingManyToOne(
				entity.getClazz(), association.getMappedBy());
		if (manyToOne != null) {
			String setName = CaseFormat.LOWER_UNDERSCORE
					.to(CaseFormat.LOWER_CAMEL, "set_" + manyToOne.getName());
			JInvocation assign1 = arg1.invoke(setName).arg(arg0);
			bodyLink.add(assign1);
		}
		String setNameThis = CaseFormat.LOWER_UNDERSCORE.to(CaseFormat.LOWER_CAMEL,
				"get_" + association.getName());
		setNameThis = "_" + setNameThis;
		JInvocation assign2 = arg0.invoke(setNameThis).invoke("add").arg(arg1);
	...
	private void unlink(JDefinedClass class_, JDefinedClass thisClass, 
		...
		String setNameThis = CaseFormat.LOWER_UNDERSCORE.to(CaseFormat.LOWER_CAMEL,
				"get_" + association.getName());
		setNameThis = "_" + setNameThis;
		JInvocation assign2 = arg0.invoke(setNameThis).invoke("remove").arg(arg1);
		...
		if (manyToOne != null) {
			String setName = CaseFormat.LOWER_UNDERSCORE
					.to(CaseFormat.LOWER_CAMEL, "set_" + manyToOne.getName());
			JInvocation assign1 = arg1.invoke(setName).arg(JExpr._null());
